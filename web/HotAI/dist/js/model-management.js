// ========== 模型管理页 ==========
let currentPage=1, pageSize=10, currentSortBy='id', currentSortOrder='desc';
let allVendors=[], allVendorsMap={};
let syncMethod='', syncLang='';
let deleteCallback=null, prefillModels=[];
...
async function loadModels() {
  ...
  if (k || v) {
    ...
    r = await apiRequest('/models/?p=' + currentPage + '&page_size=' + pageSize + '&order_by=' + currentSortBy + '&order_dir=' + currentSortOrder);
  }
  if (r.success) {
...
function renderModelTable(ms) {
  ...
  if (!ms || ms.length === 0) {
    tb.innerHTML = '<tr><td colspan="13" style="text-align:center;padding:60px 20px;"><div class="empty-state"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/></svg><div>暂无模型数据</div></div></td></tr>';
    return;
  }
  const rm = ['精确', '前缀', '包含', '后缀'];
  const rc = ['name-rule-exact', 'name-rule-prefix', 'name-rule-contains', 'name-rule-suffix'];
  const qtMap = { 0: '按量计费', 1: '按次计费' };

  tb.innerHTML = ms.map(m => {
    const vd = allVendorsMap[m.vendor_id];
    const vn = vd ? vd.name : '-';
    const lg = vd ? AIProviders.getProviderIconUrl(vd.icon || vd.name) : '';

    // Logo cell
    const logoHtml = lg
      ? `<img src="${lg}" class="provider-logo" onerror="this.style.display='none';this.nextElementSibling.style.display='inline-flex';"><span class="provider-logo-placeholder" style="display:none;">${vd.name.substring(0,2).toUpperCase()}</span>`
      : (vd ? `<span class="provider-logo-placeholder">${vd.name.substring(0,2).toUpperCase()}</span>` : '<span class="provider-logo-placeholder">?</span>');

    // Vendor capsule button (clickable to edit vendor)
    const vendorHtml = vd
      ? `<button onclick="editVendor(${vd.id})" style="padding:3px 10px;border-radius:999px;border:1px solid var(--c-border);background:var(--c-white);cursor:pointer;font-size:12px;color:var(--c-text-main);transition:all 0.15s;" onmouseover="this.style.borderColor='var(--c-primary)';this.style.color='var(--c-primary)'" onmouseout="this.style.borderColor='var(--c-border)';this.style.color='var(--c-text-main)'">${vn}</button>`
      : '<span style="color:var(--c-text-secondary);">-</span>';

    // Tags
    const tagsHtml = m.tags
      ? m.tags.split(',').filter(t=>t.trim()).map(t => `<span class="endpoint-tag">${t.trim()}</span>`).join('')
      : '-';

    // Bound channels
    const channelsHtml = (m.bound_channels && m.bound_channels.length > 0)
      ? m.bound_channels.map(ch => `<span style="display:inline-flex;align-items:center;padding:2px 7px;background:#F0FDF4;color:#16A34A;border-radius:999px;font-size:11px;margin:1px;">${ch.name}</span>`).join('')
      : '-';

    // Enable groups
    const groupsHtml = (m.enable_groups && m.enable_groups.length > 0)
      ? m.enable_groups.map(g => `<span class="badge badge-blue" style="font-size:11px;margin:1px;">${g}</span>`).join('')
      : '-';

    // Quota types
    const qtHtml = (m.quota_types && m.quota_types.length > 0)
      ? [...new Set(m.quota_types)].map(qt => `<span class="badge ${qt === 0 ? 'badge-gray' : 'badge-purple'}" style="font-size:11px;margin:1px;">${qtMap[qt] || qt}</span>`).join('')
      : '-';

    // Endpoints — backend stores as JSON array of endpoint type strings
    let ep = [];
    if (m.endpoints) {
      try {
    ...
    const epHtml = ep.length > 0 ? ep.map(e => `<span class="endpoint-tag">${e}</span>`).join('') : '-';
    ...
    return `<tr>
      ...
      <td style="text-align:center;">${vendorHtml}</td>
      <td style="text-align:center;">${tagsHtml}</td>
      <td style="text-align:center;">${channelsHtml}</td>
      <td style="text-align:center;">${groupsHtml}</td>
      <td style="text-align:center;">${qtHtml}</td>
      ...
      <td style="text-align:center;">${epHtml}</td>
...
// ===== Vendor search dropdown in model form =====
...
function filterVendorsDropdown(q) {
  ...
  if (f.length === 0) {
    ...
    d.innerHTML = f.map(v => `<div class="vendor-dropdown-item" onclick="selectVendorForModel(${v.id},'${v.name}')">${v.name}</div>`).join('');
  }
  d.style.display = 'block';
}
function selectVendorForModel(id, n) {
...
function closeConfirmModal() { document.getElementById('confirmModal').classList.add('hidden'); deleteCallback = null; }
async function doConfirmDelete() { if (deleteCallback) { await deleteCallback(); deleteCallback = null; } closeConfirmModal(); }
  ...
  if (Array.isArray(groups) && groups.length > 0) {
    l.innerHTML = groups.map(g => `
      <div class="prefill-group-item">
        <div>
          <div style="font-weight:600;margin-bottom:4px;">${g.name}
...
function openVendorModal() {
  ...
  document.getElementById('vendorIcon').value = '';
...
async function editVendor(id) {
  const r = await apiRequest('/vendors/' + id);
  if (!r.success) { showToast(r.message || '获取供应商失败', 'error'); return; }
  const v = r.data;
  ...
  document.getElementById('vendorIcon').value = v.icon || '';
  document.getElementById('vendorStatus').checked = v.status !== 0;
  document.getElementById('vendorModal').classList.remove('hidden');
}
...
async function saveVendor() {
  ...
  const d = {
    ...
    icon: document.getElementById('vendorIcon').value.trim(),
    status: document.getElementById('vendorStatus').checked ? 1 : 0
  };
  if (id) d.id = parseInt(id);
  const r = await apiRequest('/vendors/', { method: id ? 'PUT' : 'POST', body: JSON.stringify(d) });
  if (r.success) {
