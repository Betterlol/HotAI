...
当前图灵智算运行前端由 Go embed 嵌入后端二进制。仅修改这套静态前端时，不需要单独启动 React/Rsbuild 前端。

2. 编译并运行后端。

```bash
cd ~/projects/<your-project-dir>
go build -o new-api .
./new-api
```
...
适合日常开发。后端、PostgreSQL、Redis 使用 Docker Compose，当前图灵智算静态前端由后端直接服务。
...
```text
图灵智算：http://localhost:3000
```

如果专门维护 `web/default` 上游 React 前端，可单独使用 Bun 和 Rsbuild dev server；这不是当前图灵智算嵌入前端的默认开发方式。
...
图灵智算支持 pprof 和 Pyroscope 两类性能分析能力：
