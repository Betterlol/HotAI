...
图灵智算是一个基于 Go 的 AI API 网关/代理系统，聚合 OpenAI、Claude、Gemini、Azure、AWS Bedrock 等 40+ 个上游 AI 提供商，向用户提供统一的 API 接口，并包含用户管理、计费、限流、日志和管理后台。
...
个人开发者可以使用图灵智算快速接入多个模型提供商，无需为每家供应商单独编写适配逻辑。

企业团队可以通过图灵智算统一管理团队内的 API Key、额度、模型权限、审计日志和成本。

SaaS 平台可以把图灵智算作为后端 AI 服务聚合层，通过账户、分组和计费能力支撑商业化。
...
- **`i18n/`：** 后端国际化。
- **`oauth/`：** OAuth 提供商实现。
- **嵌入静态前端目录：** 当前嵌入并实际服务的图灵智算静态前端，包括 HTML、CSS、JavaScript 和 Markdown 文档。
- **`web/default/`：** new-api 上游 React 19 + Rsbuild + Base UI + Tailwind CSS 前端，当前不作为图灵智算嵌入前端入口。
- **`web/classic/`：** new-api 上游 React 18 + Vite + Semi Design 经典前端，当前不作为图灵智算嵌入前端入口。
...
- **当前运行前端：** 嵌入静态 HTML、CSS、JavaScript。
...
- 当前图灵智算静态前端使用嵌入静态前端中的 `js/i18n.js`。
...
当前图灵智算前端在嵌入静态前端目录下维护，修改 HTML、CSS、JavaScript 或 Markdown 后，需要重建后端二进制或镜像，使 Go embed 使用最新静态资源。
...
当前图灵智算静态前端的用户可见文本，应尽量沿用现有 `data-i18n` 和嵌入静态前端中的 `js/i18n.js` 机制。维护 `web/default` 上游 React 前端时，详细约束以 `web/default/AGENTS.md` 为准。
