...
flowchart LR
    Request["用户请求"]
    Request --> Existing["Token 绑定 → Affinity 命中 → CacheGetRandomSatisfiedChannel"]
    Existing --> HotAI

    subgraph HotAI["HotAI 增强选择层"]
        direction TB
        Candidates["候选渠道列表 (同 Priority)"]
        Phase1["Phase 1: 熔断过滤<br/>排除滑动窗口错误率 > 阈值<br/>半开状态按比例放行"]
        Phase2["Phase 2: 健康评分<br/>评分 = f(延迟, 成本, 成功率)"]
        Phase3["Phase 3: 加权随机选择<br/>按评分加权随机"]
        Candidates --> Phase1 --> Phase2 --> Phase3
    end
```
...
stateDiagram-v2
    [*] --> CLOSED
    CLOSED --> OPEN: 滑动窗口错误率 > 阈值
    OPEN --> HALF_OPEN: 超时后自动进入
    HALF_OPEN --> CLOSED: 连续成功数 ≥ 阈值
    HALF_OPEN --> OPEN: 放行请求失败
    CLOSED --> CLOSED: 错误率 < 阈值，保持正常
```
